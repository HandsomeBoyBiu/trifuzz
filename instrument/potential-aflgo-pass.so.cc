...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  if (is_aflgo_preprocessing) {
    ...
    ConstantInt *ReachedCtrLoc = ConstantInt::get(LargestType, MAP_SIZE + 8 * 2); // 是否已经reach the target int64
    ConstantInt *MaxFuncSeqLoc = ConstantInt::get(LargestType, MAP_SIZE + 8 * 3); // 最大函数索引     int32
    //ConstantInt *next = ConstantInt::get(LargestType, MAP_SIZE + 8 * 3 + 4);
#else
    IntegerType *LargestType = Int32Ty;
    ConstantInt *MapCntLoc = ConstantInt::get(LargestType, MAP_SIZE + 4);
    ConstantInt *ReachedCtrLoc = ConstantInt::get(LargestType, MAP_SIZE + 4 * 2); // 是否已经reach the target   int32
    ConstantInt *MaxFuncSeqLoc = ConstantInt::get(LargestType, MAP_SIZE + 4 * 3); // 最大函数索引   int32
    //ConstantInt *next = ConstantInt::get(LargestType, MAP_SIZE + 4 * 4);
#endif
    ...
    GlobalVariable *TrifuzzFuncSeqPtr = new GlobalVariable(
        M, PointerType::get(Int32Ty, 0), false, GlobalValue::ExternalLinkage, 0,
        "__trifuzz_func_seq_ptr");

    GlobalVariable *TrifuzzPotentialPtr = new GlobalVariable(
        M, PointerType::get(Int32Ty, 0), false, GlobalValue::ExternalLinkage, 0,
        "__trifuzz_potential_ptr");
    ...
    for (auto &F : M) {
      ...
      for (auto &BB : F) {
        ...
        /* if it is entry block, save current sequence */
        if (&F.getEntryBlock() == &BB) {

          /* Update Max function sequence : IncrMaxFuncSeq += 1*/
          Value *MaxFuncSeqPtr = IRB.CreateBitCast(
              IRB.CreateGEP(MapPtr, MaxFuncSeqLoc), Int32Ty->getPointerTo());
          LoadInst *MaxFuncSeq = IRB.CreateLoad(MaxFuncSeqPtr);
          MaxFuncSeq->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));

          Value *IncrMaxFuncSeq = IRB.CreateAdd(MaxFuncSeq, One);
          IRB.CreateStore(IncrMaxFuncSeq, MaxFuncSeqPtr)
              ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));

          /* Update current function sequence : __trifuzz_func_seq[func_idx] = IncrMaxFuncSeq*/
          ConstantInt *FuncIdx = ConstantInt::get(Int32Ty, func_idx);
          LoadInst *FuncSeqPtr = IRB.CreateLoad(TrifuzzFuncSeqPtr);
          FuncSeqPtr->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
          Value *FuncSeqIdx = IRB.CreateGEP(FuncSeqPtr, FuncIdx);
          IRB.CreateStore(IncrMaxFuncSeq, FuncSeqIdx)
              ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
        ...
        if (potential >= 0) {
          ...
          SAYF(cGRN "[+] Potential instrumented for function %s\n" cRST,
               F.getName().str().c_str());
          ...
          ConstantInt *FuncIdx = ConstantInt::get(Int32Ty, func_idx);
          LoadInst *PotentialPtr = IRB.CreateLoad(TrifuzzPotentialPtr);
          PotentialPtr->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
          Value *PotentialIdx = IRB.CreateGEP(PotentialPtr, FuncIdx);
          IRB.CreateStore(ConstantInt::get(Int32Ty, potential_to_instr), PotentialIdx)
              ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
        ...
        if (distance >= 0) {
          ...
          if (distance == 0) {
            /* if target is reached, ReachedCtr += 1 */
            Value *ReachedCtrPtr = IRB.CreateBitCast(
                IRB.CreateGEP(MapPtr, ReachedCtrLoc), LargestType->getPointerTo());
            LoadInst *ReachedCtr = IRB.CreateLoad(ReachedCtrPtr);
            ReachedCtr->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
            Value *IncrReachedCtr = IRB.CreateAdd(ReachedCtr, One);
            IRB.CreateStore(IncrReachedCtr, ReachedCtrPtr)
                ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
          }
