...
u8  __afl_area_initial[MAP_SIZE + 256];
//u32 __trifuzz_func_seq_initial[MAP_SIZE + 64];
//u32 __trifuzz_potential_initial[MAP_SIZE + 64];
u8 __trifuzz_critical_bb_initial[MAP_SIZE + 64];
u8* __afl_area_ptr = __afl_area_initial;
//u32* __trifuzz_func_seq_ptr = __trifuzz_func_seq_initial;
//u32* __trifuzz_potential_ptr = __trifuzz_potential_initial;
u8* __trifuzz_critical_bb_map_ptr = __trifuzz_critical_bb_initial;
...
static void __afl_map_shm(void) {
  ...
  u8 *id_str2 = getenv(TRIFUZZ_CRITICAL_BB_ENV_VAR);
  if (id_str2) {
    u32 shm_id = atoi(id_str2);
    __trifuzz_critical_bb_map_ptr = shmat(shm_id, NULL, 0);
    if (__trifuzz_critical_bb_map_ptr == (void *)-1) _exit(1);
  }

  //u8 *id_str3 = getenv(TRIFUZZ_POTENTIAL_ENV_VAR);
  //if (id_str3) {
  //  u32 shm_id = atoi(id_str3);
  //  __trifuzz_potential_ptr = shmat(shm_id, NULL, 0);
  //  if (__trifuzz_potential_ptr == (void *)-1) _exit(1);
  //}
...
int __afl_persistent_loop(unsigned int max_cnt) {
  ...
  if (first_pass) {
    ...
    if (is_persistent) {
      ...
      memset(__afl_area_ptr, 0, MAP_SIZE + 256);
      __afl_area_ptr[0] = 1;
      __afl_prev_loc = 0;

      memset(__trifuzz_critical_bb_map_ptr, 0, MAP_SIZE + 64);
      //memset(__trifuzz_potential_ptr, 0, (MAP_SIZE + 64) * 4);
    }
  ...
  if (is_persistent) {
    ...
    if (--cycle_cnt) {
      ...
      __trifuzz_critical_bb_map_ptr = __trifuzz_critical_bb_initial;
      //__trifuzz_potential_ptr = __trifuzz_potential_initial;
